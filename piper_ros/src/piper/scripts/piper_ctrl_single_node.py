...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            data1 = [0x05, 0x70, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00]
            self.piper.end_can_send(cmd_id, data1)

            enable_id = 0x0300FD7F
            data2 = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
            self.piper.end_can_send(enable_id, data2)

            data3 = [0x18, 0x70, 0x00, 0x00, 0x00, 0x00, 0x20, 0x41]
            self.piper.end_can_send(cmd_id, data3)

            data4 = [0x22, 0x70, 0x00, 0x00, 0x00, 0x00, 0xA0, 0X41]
            self.piper.end_can_send(cmd_id, data4)

            data5 = [0x0A, 0x70, 0x00, 0x00, 0xC3, 0xF5, 0x48, 0x40]
            self.piper.end_can_send(cmd_id, data5)

            rospy.loginfo("末端电机使能成功")
        except Exception as e:
            rospy.logerr(f"end motor enable failed: {e}", exc_info=True)
