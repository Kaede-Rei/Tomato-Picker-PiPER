...
#include "piper_contract/CommandOctomap.h" // IWYU pragma: keep
...
public:
    ...
    CloudProprocessor()
        : nh_(), pnh_("~"),
        color_sub_(nh_, get_param<std::string>("topics/color_image", "/camera/wrist/color/image_raw"), 1),
        depth_sub_(nh_, get_param<std::string>("topics/depth_image", "/camera/wrist/depth_registered/image_raw"), 1),
        depth_info_sub_(nh_, get_param<std::string>("topics/depth_info", "/camera/wrist/depth_registered/camera_info"), 1),
...
private:
    ...
    bool on_request(piper_contract::CommandOctomapRequest& req, piper_contract::CommandOctomapResponse& res) {
